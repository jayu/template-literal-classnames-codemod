...
const _createObjectExpression = j => (entries) => {
  return j.objectExpression(entries.map(([property, value]) => j.objectProperty.from({
    key: property,
    value,
    computed: true
  })))
...
const _getClassNamesIdentifierName = (j) => (ast) => {
  ...
  if (importDeclarations.length === 1) {
    ...
    const defaultImport = j(importDeclaration).find(j.ImportDefaultSpecifier).get()
...
module.exports = (fileInfo, api, options) => {
  ...
  const transformConditionalExpression = options.hasOwnProperty('conditionalExp')
  const transformFalsyConditionalExp = options.hasOwnProperty('falsyConditionalExp')
  const classAttrName = ['className', ...((options.classAttrName || '').split(','))]

  const existingClassNamesImportIdentifer = getClassNamesIdentifierName(ast)
  const classNamesImportName = existingClassNamesImportIdentifer || options.classnamesImport || CLASSNAMES_IDENTIFIER_NAME;
  ...
  classAttrName.forEach((classAttrName) => {
    ...
    classNameAttrs.forEach((path) => {
      ...
      if (transformConditionalExpression) {
        cxArguments = cxArguments.map((arg) => {
          if (arg.type === 'ConditionalExpression') {
            ...
            const optionalLogicalExpression = (previousCondition, currentCondition) => {
              if (previousCondition === null) {
                return currentCondition
              }
              return j.logicalExpression('&&', previousCondition, currentCondition)
            }

            const transformConditionalExpression = (expression, previousCondition = null) => {
              const expressionsList = [];

              const currentCondition = optionalLogicalExpression(previousCondition, expression.test)
              const currentNegatedCondition = optionalLogicalExpression(previousCondition, j.unaryExpression('!', expression.test))

              if (expression.consequent.type === 'ConditionalExpression') {
                expressionsList.push(...transformConditionalExpression(expression.consequent, currentCondition))
              }
              else if (!isFalsyNodeValue(expression.consequent)) {
                expressionsList.push([expression.consequent, currentCondition])
              }
              if (expression.alternate.type === 'ConditionalExpression') {
                expressionsList.push(...transformConditionalExpression(expression.alternate, currentNegatedCondition))
              }
              else if (!isFalsyNodeValue(expression.alternate)) {
                expressionsList.push([expression.alternate, currentNegatedCondition])
              }
              return expressionsList;
            }

            return createObjectExpression(transformConditionalExpression(arg))
          }
          return arg
        })
      }
      ...
      if (shouldUseCX) {
        j(templateLiteral).replaceWith(createCxCallExpression(cxArguments, classNamesImportName));
        shouldInsertCXImport = true;
      } else {
        const className = cxArguments[0]
        if (className.type === 'Literal') {
          j(templateLiteral).closest(j.JSXExpressionContainer).replaceWith(className);
        }
        else {
    ...
    if (transformFalsyConditionalExp) {
      ...
      classNameAttrs.forEach((path) => {
        const conditionalExpression = j(path).find(j.ConditionalExpression).get()
        const conditionalExpressionNode = conditionalExpression.node
        if (isFalsyNodeValue(conditionalExpressionNode.consequent)) {
          shouldInsertCXImport = true;
          j(conditionalExpression).replaceWith(createCxCallExpression([
            createObjectExpression([
              [
                conditionalExpressionNode.alternate,
                j.unaryExpression('!', conditionalExpressionNode.test)
              ]
            ])
